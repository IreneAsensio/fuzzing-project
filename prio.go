...
type SyscallDep struct {
	Name         string   `json:"name"`
	Dependencies []string `json:"dependencies"`
}

func GetDependencyMap(filePath string) (map[string][]string, error) {
	...
	var syscallDeps []SyscallDep
	if err := json.Unmarshal(content, &syscallDeps); err != nil {
		return nil, fmt.Errorf("error al decodificar el contenido del archivo: %v", err)
	}

	// Crear un mapa para almacenar las dependencias
	dependencyMap := make(map[string][]string)

	// Recorrer las dependencias y almacenarlas en el mapa
	for _, dep := range syscallDeps {
		dependencyMap[dep.Name] = dep.Dependencies
	}
...
func (target *Target) CalculatePriorities(corpus []*Prog) [][]int32 {
	...
	dependencyMap, err := GetDependencyMap("implicit-dep-output.json")
	if err != nil {
		log.Fatalf("error al obtener el mapa de dependencias: %v", err)
	}
	if len(corpus) != 0 {
		dynamic := target.calcDynamicPrio(corpus, dependencyMap)
	...
	for _, weights := range uses {
		for _, w0 := range weights {
			for _, w1 := range weights {
				if w0.call == w1.call {
					//log.Logf(0, "wo.call %v", w0.call)
					//log.Logf(0, "target.syscalls %v", target.Syscalls)
...
func (target *Target) calcDynamicPrio(corpus []*Prog, dependencyMap map[string][]string) [][]int32 {
	...
	for _, p := range corpus {
		for _, c0 := range p.Calls {
			...
			dependencies, ok := dependencyMap[c0.Meta.Name]
			if ok {
				secondSyscall := dependencies[1] // we suppose that there is only one dependency
				for _, c1 := range p.Calls {
					if c1.Meta.Name == secondSyscall {
						prios[c0.Meta.ID][c1.Meta.ID] = prioHigh
					}
					else {
						prios[c0.Meta.ID][c1.Meta.ID]++ //if no dependencies are found, we leave the original implementation
					}				
				}
			}
			else {	
				for _, c1 := range p.Calls {
					prios[c0.Meta.ID][c1.Meta.ID]++ //the first syscall has no dependencies so we leave the original implementation
				}
